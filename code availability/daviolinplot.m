...
% This code is obtained online from https://doi.org/10.5281/zenodo.12749045
% povilaskarvelis/DataViz: v3.2.4
% Authors/Creators: Povilas Karvelis, oyvindlr
...
if ~isempty(confs.xvalues)

    % user-defined x positions
    gpos = confs.xvalues;

    % check dimensions
    if ~isequal(size(gpos),[num_groups,num_locs])
        error(['xvalues must be a num_groups × num_conditions matrix. ' ...
               'Expected size: %d × %d'],num_groups,num_locs);
    end

else

    % original automatic positioning
    gpos=[];
    ...
    if num_locs==1
        ...
        if num_groups==1
            ...
            for g = 1:num_groups
                gpos = [gpos; ...
                    cpos + (g-(num_groups+1)/2)*(box_width + loc_sp)];
            end
        end
    end
end
